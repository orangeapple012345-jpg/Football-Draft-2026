...
function calcFullPhases(sqA,sqB){
  ...
  // ── ATTACK ───────────────────────────────────────────────
  // Base from attack cohesion (uses real forwards)
  let aAtk=CA.totalAttackCohesion;
  // Adjust for how well B's defence neutralises A's threats
  ...
  aAtk+=stAdvantageA*0.22+lwAdvantageA*0.12+rwAdvantageA*0.12+camAdvantageA*0.08;
  // GK quality damps attack
  aAtk-=clamp2(TB.gkShotStopping-74,0,20)*0.12;
  // Synergy boosts attack
  aAtk+=clamp2(synA.rawBonus*0.15,0,10);
  aAtk=clamp2(aAtk,30,99);
  ...
  bAtk+=stAdvantageB*0.22+lwAdvantageB*0.12+rwAdvantageB*0.12+camAdvantageB*0.08;
  bAtk-=clamp2(TA.gkShotStopping-74,0,20)*0.12;
  bAtk+=clamp2(synB.rawBonus*0.15,0,10);
  bAtk=clamp2(bAtk,30,99);

  // ── MIDFIELD ─────────────────────────────────────────────
  let aMid=avg2(posScore2(TA.cdm,"cdm"),posScore2(TA.cm,"cm"),posScore2(TA.cam,"cam"));
  aMid+=clamp2(CA.effectiveMidfieldCreativity-65,0,15)*0.25;
  aMid+=clamp2(CA.effectivePressResistance-TB.teamPressing,-15,15)*0.3;
  aMid+=clamp2(TA.teamWorkRate-70,0,18)*0.15;
  aMid+=clamp2(TA.teamIntelligence-78,0,15)*0.14;
  aMid+=(TA.teamPhysical-TB.teamPhysical)*0.08;
  aMid=clamp2(aMid,35,99);
  ...
  // Penalty for having no CDM screen
  if(!TA.cdm)aDefStr-=10;
  // Penalty for attacking FB risk without coverage
  aDefStr-=CA.leftFlankDefGap*0.5+CA.rightFlankDefGap*0.5;
  // GK bonus
  aDefStr+=clamp2(TA.gkShotStopping-72,0,20)*0.16;
  aDefStr=clamp2(aDefStr,25,99);

  let bDefStr=CB.totalDefCoverage;
  if(!TB.cdm)bDefStr-=10;
  bDefStr-=CB.leftFlankDefGap*0.5+CB.rightFlankDefGap*0.5;
  bDefStr+=clamp2(TB.gkShotStopping-72,0,20)*0.16;
  bDefStr=clamp2(bDefStr,25,99);
...
// ═══════════════════════════════════════════════════════════════════════════
//  STEP 6: DYNAMIC SCORELINE GENERATOR
//  Based on actual attack/defence phase scores — can produce 0-0 to 9-0
// ═══════════════════════════════════════════════════════════════════════════
function dynamicScoreline(aAtk,bAtk,aDefStr,bDefStr,aGK,bGK,synA,synB){
  // Expected goals = function of attacking power vs defensive resistance
  // Scale: attack 99 vs defence 30 = ~4.5 goals. Attack 60 vs defence 90 = ~0.2 goals
  function xgCalc(atk,def,gk){
    const atkNorm=(atk-40)/59; // 0-1
    const defNorm=(def-40)/59; // 0-1
    const gkFactor=clamp2(1-(gk-55)/80,.45,1.0);
    // Base XG — attack dominance creates goals
    let xg=atkNorm*3.8-defNorm*2.6;
    xg=clamp2(xg,0,5.5);
    xg*=gkFactor;
    return xg;
  }
  const xgA=xgCalc(aAtk,bDefStr,bGK);
  const xgB=xgCalc(bAtk,aDefStr,aGK);

  // Add synergy variance — a cohesive team converts more of their chances
  const synFactorA=clamp2(synA/100,0.6,1.2);
  const synFactorB=clamp2(synB/100,0.6,1.2);
  const xgAFinal=xgA*synFactorA;
  const xgBFinal=xgB*synFactorB;

  // Poisson-like sampling — convert XG to goals with variance
  function sampleGoals(xg){
    if(xg<=0)return 0;
    // High XG has variance — a 4.0 XG could produce 2 or 6 goals
    const variance=rng2(-xg*0.35,xg*0.4);
    const raw=Math.max(0,xg+variance);
    // Bias toward whole numbers but allow fractional rounding
  ...
  // Clamp to prevent unrealistic scorelines (max 8 goals per team)
  goalsA=clamp2(goalsA,0,8);
  goalsB=clamp2(goalsB,0,8);
...
function dynamicPlayerRatings(sqA,sqB,phases){
  ...
  function ratePlayer(p,role,myPhases,oppPhases,myTeamT,oppTeamT,myCoverage){
    ...
    // Convert pos score (40-99) to rating (4.0-9.8)
    let r=4.0+((base-40)/59)*5.4;

    // Context: how did your role's phase go?
    let phaseBoost=0;
    if(["st","lw","rw"].includes(role))phaseBoost=(myPhases.aAtk-oppPhases.bAtk)*0.055;
    else if(["cdm","cm","cam"].includes(role))phaseBoost=(myPhases.aMid-oppPhases.bMid)*0.05;
    else if(["cb","lb","rb"].includes(role))phaseBoost=(myPhases.aDefStr-oppPhases.bDefStr)*0.045;
    else if(role==="gk")phaseBoost=(oppPhases.bAtk-myPhases.aAtk)*0.04; // GK rates higher under more pressure
    r+=clamp2(phaseBoost,-2.0,2.0);

    // Consistency bounds the variance — inconsistent players vary widely
    const cons=a(p,"consistency")/100;
    const maxVariance=(1-cons)*1.4;
    r+=rng2(-maxVariance,maxVariance);

    // Big game factor — elite players are more likely to have big performances
    const bgRating=a(p,"bigGameRating")/100;
    const bgFloat=rng2(-0.3,bgRating*0.55);
    r+=bgFloat;

    // Role-specific bonuses
    if(role==="cdm"&&myCoverage.cdmCanCoverFBs)r+=0.25; // rewarded for good coverage
    if(role==="cm"&&myCoverage.cmDropsToScreen)r+=0.2;
    if(role==="gk"&&oppPhases.bAtk>75)r+=rng2(0,0.4); // under fire GK can rise to occasion

    return clamp2(Math.round(r*10)/10,3.0,9.9);
  }
